			...
			if operand.Kind == "" {
				...
				if err == nil {
					...
					if registryInstance.Spec.Noolm == false {
						...
						err = r.reconcileCRwithConfig(ctx, opdConfig, configInstance.Name, configInstance.Namespace, csv, requestInstance, operand.Name, configInstance.Namespace, &r.Mutex)
						if err != nil {
							merr.Add(err)
							requestInstance.SetMemberStatus(operand.Name, "", operatorv1alpha1.ServiceFailed, &r.Mutex)
							continue
				...
				if registryInstance.Spec.Noolm == false {
					...
					err = r.reconcileCRwithRequest(ctx, requestInstance, operand, types.NamespacedName{Name: requestInstance.Name, Namespace: requestInstance.Namespace}, i, requestInstance.Namespace, &r.Mutex)
					if err != nil {
						merr.Add(err)
						requestInstance.SetMemberStatus(operand.Name, "", operatorv1alpha1.ServiceFailed, &r.Mutex)
						continue
