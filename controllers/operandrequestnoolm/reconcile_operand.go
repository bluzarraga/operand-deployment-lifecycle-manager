...
func (r *Reconciler) reconcileOperand(ctx context.Context, requestInstance *operatorv1alpha1.OperandRequest) *util.MultiErr {
	...
	for _, req := range requestInstance.Spec.Requests {
		...
		for i, operand := range req.Operands {
			...
			klog.V(3).Info("Generating customresource base on Operand: ", operand.Name)
			...
			if operand.Kind == "" {
				...
				if err == nil {
					...
					err = r.reconcileCRwithConfig(ctx, opdConfig, configInstance.Name, configInstance.Namespace, requestInstance, operand.Name, registryKey.Namespace, &r.Mutex)
					if err != nil {
						merr.Add(err)
				...
				err = r.reconcileCRwithRequest(ctx, requestInstance, operand, types.NamespacedName{Name: requestInstance.Name, Namespace: requestInstance.Namespace}, i, registryKey.Namespace, &r.Mutex)
				if err != nil {
					merr.Add(err)
					requestInstance.SetMemberStatus(operand.Name, "", operatorv1alpha1.ServiceFailed, &r.Mutex)
					continue
...
// reconcileCRwithConfig merge and create custom resource base on OperandConfig
func (r *Reconciler) reconcileCRwithConfig(ctx context.Context, service *operatorv1alpha1.ConfigService, opConfigName, opConfigNs string, requestInstance *operatorv1alpha1.OperandRequest, operandName string, operatorNamespace string, mu sync.Locker) error {
	merr := &util.MultiErr{}
	...
	for cr := range service.Spec {
		...
		if err != nil && !apierrors.IsNotFound(err) {
			...
			if err := r.compareConfigandExample(ctx, cr, service, opConfigNs); err != nil {
				merr.Add(err)
				continue
			}
		} else {
			if r.CheckLabel(cr, map[string]string{constant.OpreqLabel: "true"}) {
				// Update or Delete Custom Resource
				if err := r.existingCustomResource(ctx, cr, spec.(map[string]interface{}), service, opConfigNs); err != nil {
					merr.Add(err)
					continue
				}
				statusFromCR, err := r.getOperandStatus(cr)
				if err != nil {
					return err
				}
				serviceKind := cr.GetKind()
	...
	for cr, found := range foundMap {
		if !found {
			klog.Warningf("Custom resource %v doesn't exist in the alm-example of %v", cr, operand.Name)
		}
	}
